...
def create_gantt(input_file):
    

    # Create resources and tasks from CSV
    chart = Chart(input_file)
    project = Project(input_file)
    chart.add_project(project)
    curretdatetime=datetime.now().strftime("%Y%m%d-%H%M%S")
    output_file = 'gantt_chart'+str(curretdatetime)+'.svg'
    resources = {}
    tasks = {}
...
if __name__ == '__main__':
    ...
    tk.Button(root,text='Gantt Chart',command=get_gantt).grid(row=1, column=1)
